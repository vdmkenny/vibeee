...
| web | [`web/`](web/) | A reader for pages. It fetches a page over HTTP or HTTPS, sets its words in a column in the system's own faces, and follows its links. Of what a page's stylesheets say it follows what a column of text can show: what they hide, the colours of words and of what they sit on, and which way lines lean, read for the window it is drawn in. A form is filled in with the toolkit's own controls and sent: one that asks by GET in the address, the way a search does, and one that asks by POST in the body of a request, the way logging in and ordering are done. A page's pictures follow its words one at a time, what the page says each one shows standing in until it arrives. A page's scripts run in the reader, on the tree the page was read from, under bounds on what they may hold, how deep they may call and how long each may run; what they change is read again and drawn, and what they ask for from outside the page is answered on a later pass, so nothing a script does waits on the network. `web -t <address>` prints a page's words in the shell. Its settings are the `web` domain, `cfg web`, and the menu at the end of its strip changes them, whether pages are drawn light or dark among them, and makes the page on screen the home page. |
