...
- No environment: `getenv` answers null, and `HOME` and the program search path are
  constants in the shell. A ported program that looks for its data by environment
  variable has to be told where to look another way.
