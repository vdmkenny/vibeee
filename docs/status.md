...
| `sndd` | [`user/sndd/`](../src/user/sndd/) | The sound service. A routing graph (`lib/audiograph`) sits in the middle: every program that makes or takes sound is a node with ports, the hardware is a node like any other, and links decide who hears whom. Fan-in mixes, fan-out copies, defaults point at the hardware ins and outs until repointed. Frames ride shared `lib/spsc` rings; the channel carries only the graph's verbs. The pace is the hardware's period interrupt, one bounded mix per wake, nothing polled. Two drivers share `pcm.zig`, which holds what every PCM controller needs and none should write twice: DMA arenas, bounded settling waits, period slicing, and turning a hardware position into "how many periods finished". `ac97` runs the Intel controller's DMA engines over a 32-entry descriptor ring. `hda` runs the High Definition Audio controller and walks its codec's widget graph: nothing about the analog path is assumed, so an output pin with a converter behind it is found by following connections, and that path is what gets powered, unmuted at its own full scale, and pointed at a stream. Both were verified against the emulator's wav capture at the right frequency and amplitude. `tone`, `vol` and `patch` are the tools. |
| `usbd` | [`user/usbd/`](../src/user/usbd/) | The USB bus. One event loop over the service channel, the controllers' interrupts and the volumes' doorbells; nothing polls, and a bus with nothing happening on it costs nothing. Two controller drivers sit behind one seam: `ehci.zig` for high speed, which takes the controller from the firmware by the specification's handshake and runs an asynchronous ring for control and bulk transfers and a periodic list for the endpoints hardware polls on its own behalf; and `uhci.zig` for the companions a full or low speed root port belongs to, which is the older and simpler design, registers in I/O space and descriptors of one packet each. Which of them a device is on changes nothing above. Registers are packed structs with their bit positions proven at compile time. `core.zig` enumerates: reset a port, learn its packet size, hand it an address, read what it says it is, and ask the device manager which driver fits, so a class this build has never met is a manifest and a program away. `umass.zig` drives disks over the bulk-only transport, `hid.zig` keyboards and mice in the boot protocol every one of them speaks. A disk becomes a volume the kernel's filesystems mount; a key becomes a key, meaning whatever the layout says. `usb` is the tool. |
...
| `edit` | [`user/tools/edit.zig`](../src/user/tools/edit.zig) | The editor. What it does with characters is `lib/text`, and what it does with the screen is the pager's lower half, which both it and `page` draw through: take the screen, draw a band of rows, put a bar on the last one, read a key. What is left is the arrangement. Follows the cursor in both directions, so a line wider than the console scrolls sideways rather than wrapping and pushing the layout down. Nothing is written until asked, and a file too long to hold is opened but refused a save rather than written back as its own head. |
...
| `eeelibc` | Done enough to build and run a POSIX program: `examples/greet.c` compiles and runs against it, exercising arguments, formatted output, allocation, string handling and the screen size. No `fork`, no asynchronous signals, no sockets, no float conversions. The raw-terminal path (`tcgetattr`/`tcsetattr`, `VMIN`) is implemented and no longer has a program exercising it, since the editor that did is gone and its replacement is Zig |
...
- No hubs: a device behind one is not seen, so a keyboard plugged into a monitor
  or a dock does not appear.
