...
`needs`/`provides`, registered settled) is load-bearing. What M2 still owes on USB is
the UHCI companions, which is what a full or low speed keyboard on a root port needs.
M3 owes the GUI apps parked there.
...
- USB is EHCI only, so a full or low speed device on a root port is handed to a
  companion controller nothing drives yet. In QEMU, whose EHCI carries such devices
  itself, a keyboard and a mouse work; on the machine they need the UHCI driver.
- No hubs: a device behind one is not seen.
