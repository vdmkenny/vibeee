...
| Framebuffer console | [`drv/video/fbcon.zig`](../src/drv/video/fbcon.zig) | 32bpp, Spleen font, pixel rectangles for the panic QR, a drawn cursor, and a cell grid that carries content across a mode change. Writing changes the cell grid and nothing else; `present` reconciles the screen with the grid, comparing cell against cell, and runs once per write rather than once per line, so pouring a page of text costs one screen of glyphs instead of one per row. One writer renders at a time: an interrupt landing inside another context's half-drawn line keeps the log record and the serial mirror but leaves the pixels alone. Debug boots keep two corner tells for reading a frozen photograph: the heartbeat glyph, which moves as long as timer interrupts arrive, and beside it the last interrupt vector taken, bright while its handler runs and dimmed when it completed, so a freeze names the context that died. |
...
- **The radio joins a network; it does not stay in one yet.** Scanning and
  ...
  and took a DHCP lease. Carrying traffic is unconfirmed: a ping to the gateway
  drew no answer, and the bar's network icon stayed dim while `net` reported
  the interface up with its address.

  What is missing is management frame protection (802.11w). Nothing the station
  acts on is authenticated, so a forged deauthentication or disassociation ends
  the join; it now ignores one not addressed to it and takes one answer of each
  kind per exchange, but a forgery that arrives before the real one is still
