...
| [`entropy.zig`](../src/lib/entropy.zig) | Unpredictable bytes, gathered from what the machine cannot foresee. A radio hears a room nobody arranged, and a frame its baseband could not decode is that room's noise written down. Stirred through a hash and drawn without handing back what is in it. The clock stands in where nothing has been heard, which is weaker and said to be. |
...
| [`kind.zig`](../src/lib/kind.zig) | What a file is, through two doors onto one answer: the name, for a listing asking about every row a cursor passes over, and the bytes, for anything that can afford to read before it acts. Wider than what this build can open, so a zip is a zip rather than a shrug; a container is told from what is inside it, and every kind belongs to a family a caller can act on. Pure, host-tested. |
| [`openers.zig`](../src/lib/openers.zig) | Which program opens what, as a bit per family beside the program itself. The choice is a setting over it, and one that names something gone falls back rather than failing. Pure, host-tested. |
...
- **The radio hears and speaks; it has not joined a network yet.** Scanning is
  ...
  What does not work is the exchange. The access point answers the
  authentication request correctly, and the join does not act on the answer.
  What is known about that answer, from the machine: it carries sequence two
  and a success status, and its cell address is the one being joined, which is
  every test the join applies to it. What is not known is why it is then
  ignored. The station reports which step it gave up on and which step it was
  on when the answer arrived, and those two together say whether the answer
  landed in a state that looks at it.

  One thing is unexplained alongside: three attempts are made and two frames
  go out. Both symptoms would follow from the exchange not being on the step it
  is thought to be on when the service next runs it, which is what the two
  reports are there to settle.
