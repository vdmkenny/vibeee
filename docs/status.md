...
| [`wpa2.zig`](../src/lib/wpa2.zig) | WPA2 with a pre-shared key as arithmetic, each piece against the standard's own vectors: the master key from a passphrase, the transient key from the nonces, the key-frame integrity code, the group-key wrap, the counter-mode cipher under every data frame, and the four-way handshake as a value a test can hand key frames to. |
| [`ar5212.zig`](../src/lib/ar5212.zig) | The AR5212 family's pure vocabulary: the descriptor's words, chain arithmetic, hardware rate codes, the synthesizer word, the delta-slope coefficients, the analog bank serialiser, the noise-floor history, the I/Q correction, the spur-channel test, the rate-duration table, and the calibration store's whole header layout, read through one call so a synthetic store is read on the host. |
