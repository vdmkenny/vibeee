...
- **The radio hears and speaks; it has not joined a network yet.** Scanning is
  ...
  What does not work is the exchange itself: the access point acknowledges the
  authentication request and no answer follows within the three attempts the
  join makes, so it reports and comes round again. Acknowledgement is a
  hardware reflex to a well-formed unicast frame and says only that the frame
  arrived, so what is not yet known is whether the answer is never sent, never
  received, or received and refused. The station counts all three: frames heard
  while an exchange is in hand, how many were addressed to this station, how
  many were authentications, and how many sends it asked for against how many
  the radio reports going out. The radio answers alongside with what its queue
  did and what its registers hold.
