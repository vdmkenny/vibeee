...
- A machine that sleeps is still a machine that cannot: there is no S3 path at all,
  and only the state that means off is named. What the bus's half of a resume would
  be is written and works, `usb rebuild` being that path asked for by hand, so what
  is missing is the sleep rather than the waking.
